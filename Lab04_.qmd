...
*Normal Distribution*
A t-test mandates data with a normal distribution. Here we can use a visual method to access the normality of the data:
...
######## after the transformations, we can see that the cutthroat trout weight is mugh higher in clear cut forests, when it initially
### looked like means weights were higher in old growth forests, from the first graph
...
and_vertebrates |> 
  ...
  ###### reject the null hypothesis, the small p-value (< 2.2e-16) indicates a statistically significant 
  ### relationship between forest type and channel unit
```
...
clearcut_weights <- sally_clean |> 
  filter(section == "CC") |> 
  pull(weight_g)

oldgrowth_weights <- sally_clean |> 
  filter(section == "OG") |> 
  pull(weight_g)

var.test(clearcut_weights, oldgrowth_weights)

ggpubr::ggarrange(ggpubr::gghistogram(cc_weight, main = "Clear Cut"), 
                  ggpubr::gghistogram(og_weight, main = "Old Growth"))

### small p-value (1.439e-07) indicates that the mean weights are significantly different
### data is not normally distributed, skewed to the right quite dramatically



####

var.test(log(clearcut_weights), log(oldgrowth_weights))

######## p-value 0.005299


t.test(log(sally_clean$weight_g) ~ sally_clean$section, var.equal = TRUE)


##### p-value (< 2.2e-16) is very small. We should reject the null hypothesis
#### Coastal giant salamander weights are higher in clear cut forests
...
### before, data was skewed right, now has 2 peaks

cor.test(log(trout_clean$length_1_mm), log(trout_clean$weight_g))


####### very strong correlation by small p-value (< 2.2e-16) and correlation estimate of .992
...
ggplot(pie_crab, aes(x = site, y = size, fill = site)) +
  geom_boxplot() +
  labs(title = "Carapace Width Across Sites, by Latitude",
       x = "Site",
       y = "Carapace Width (mm)") +
  theme_minimal()

#### the data shows that there is a significant difference in size depending on the site. The site PIE has the largest sizes overall, 
###  followed by GTM and DB. The ANOVA test resulted in a small p-value (< 2e-16), indicating that the differences in size between
### sites is statistically significant.
...
```{r}
pie_crab_lm <- lm(size ~ water_temp_sd, data = pie_crab)
...
#### due to the p-value (0.53) we do not reject the null hypothesis that water temperature does not have a 
### statistically significant impact on carapace width.
...
pie_crab_mulreg <- lm(size ~ latitude + air_temp_sd + water_temp_sd, data = pie_crab)

summary(pie_crab_mulreg)


##### the multiple regression model shows that each of these factors (latitude, air temperature, and water temperature)
### do have a significant impact on size, with a very small p-value (< 2.2e-16) indicating this statistical significance.
```
